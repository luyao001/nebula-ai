...
- **仓库智能**：授权工作目录后可浏览文件树、按文件名与内容全文检索、只读打开文件，并把文件路径一键引用到任务输入。
- **Git 改动审查**：任务之外提供工作区级的暂存/未暂存统一 diff 审查视图，与浏览器、文件树并列。
- **`@文件` 上下文**：在任务里用 `@src/main.ts` 这样的路径引用文件，Nova 会把内容（自动截断）附加给模型作为上下文。
- **安全本地工具**：文件读写、全文搜索、受限命令和 URL 获取均由 Rust 网关校验；敏感操作需要明确授权。
- **防覆盖写入**：`write_file` 授权时记录文件指纹，审批后目标若被外部修改则拒绝写入，避免过期 diff 被盲目应用。
- **透明授权**：`write_file` 授权弹窗展示逐行 diff（长差异折叠为上下文片段），`run_command` 高亮白名单命中原因，`fetch_url` 展示域名与 DNS 解析摘要。
- **执行可观测**：实时展示 Agent 状态与累计 token 用量，工具输出与网页自检诊断（按严重度着色）可在时间线中按需查看；任务失败或中断后可一键重试。
- **网页自检**：Agent 网页模式在禁网沙盒中执行 DOM 与运行时检查，最多自动修复三轮。
- **任务快照 v2**：保存计划、最近 120 条对话、脱敏工具时间线、累计 token 用量与完整/部分产物；重启后中断任务恢复为"已停止"并可继续，失败时的部分成果不再丢失。
...
从 [Releases](https://github.com/luyao001/nova/releases) 下载最新安装包：

| 平台 | 产物 |
| --- | --- |
| Windows 10/11 x64 | NSIS `.exe` 安装程序 |
| macOS 11+ Apple Silicon | `.dmg` |
| macOS 11+ Intel | `.dmg` |

发布由 GitHub Actions 从标签构建，附 `SHA256SUMS.txt` 与构建来源证明；详见 [RELEASING.md](RELEASING.md)。
...
任务输入支持 `@相对路径` 引用工作区文件：Nova 会读取内容并附加到模型上下文（单文件超过 24,000 字符自动截断，最多 8 个引用），时间线会标注已附加的文件。Agent 也可以用 `search_files` 工具按文件名或内容检索工作区，避免盲目遍历目录。
...
Ollama 模式直接访问本机服务。OrcaRouter 模式会把当前对话发送到所选云端模型；API Key 只驻留内存，不会持久化。导出的文件只写入用户在保存对话框中选择的位置。Agent 任务快照不包含 API Key，但会保存最近 120 条对话消息（上限 512 KB）以便恢复任务；普通单轮生成的对话仍只保留在内存中。
